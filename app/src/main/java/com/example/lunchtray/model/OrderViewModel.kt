...
class OrderViewModel : ViewModel() {
    ...
    fun setEntree(entree: String) {
        ...
        if (_subtotal.value!=null) previousEntreePrice = _entree.value!!.price - previousEntreePrice


        // TODO: set the current entree value to the menu item corresponding to the passed in string
       for (element in menuItems){
           if (element.value.name == entree){
               _entree.postValue(element.value)
               _subtotal.postValue(element.value.price)
           }
    ...
    fun setSide(side: String) {
        ...
        if (_subtotal.value!=null) previousSidePrice = _side.value!!.price - previousSidePrice
        ...
        for (element in menuItems){
            if (element.value.name == side){
                _side.postValue(element.value)
                _subtotal.postValue(element.value.price)
            }
        }
    }
    ...
    fun setAccompaniment(accompaniment: String) {
        ...
        if(_subtotal.value!=null) previousAccompanimentPrice = _accompaniment.value!!.price - previousAccompanimentPrice
        ...
        for (element in menuItems){
            if (element.value.name == accompaniment){
                _accompaniment.postValue(element.value)
                _subtotal.postValue(element.value.price)
            }
        }
    }
    ...
    private fun updateSubtotal(itemPrice: Double) {
        ...
        if(_subtotal.value!=null) _subtotal.postValue(itemPrice) else _subtotal.postValue(previousEntreePrice)
